...
BOT_TOKEN = "8945330561:AAGlwKjKRNboeessiycE8K4NPUUQHfFTBes"
GITHUB_TOKEN = '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'
REPO_OWNER = "khtin762-creator"
REPO_NAME = "mybot"
...
async def auto_rotate_loop():
    while proxy_enabled and _tor_processes:
        ...
        if not proxy_enabled:
            break
        await rotate_tor()
        pass  # normal status output suppressed
...
# Register ID များကို GitHub မဟုတ်ဘဲ SD card/local storage ထဲတွင် သိမ်းမည်။
# Android/Termux တွင် /sdcard ရှိပါက ထိုနေရာကိုသုံးပြီး၊
# အခြား server တွင် ./auth_list.json ကို fallback အဖြစ်သုံးသည်။
AUTH_FILE = os.environ.get(
    "AUTH_FILE",
    "/sdcard/register_ids.json" if os.path.isdir("/sdcard") else "register_ids.json"
)
...
def load_local_auth():
    try:
        ...
        print(f"[auth] read error ({AUTH_FILE}): {e}")
        return {}

def save_local_auth(auth_list):
...
async def get_auth_list():
    return load_local_auth()

async def save_auth_list(auth_list):
    save_local_auth(auth_list)
...
async def start(message):
    await bot.reply_to(
        message,
        "Bot စတင်ပါပြီ။\n\n"
        "Register လုပ်ထားသော key ဖြင့် ဝင်ရန်:\n"
        "/key [your_key]\n\n"
        "ဥပမာ:\n"
        "/key ABC123"
    )
...
async def handle_key(message):
    ...
    auth_list = await get_auth_list()

    # botvip.py နှင့်တူအောင် လက်ခံမည့်ပုံစံများ:
    # 1) ကိုယ့် Telegram User ID ကို key အဖြစ်သုံးခြင်း
    # 2) auth_list ထဲတွင် User ID အဖြစ် register လုပ်ထားခြင်း
    # 3) auth_list ထဲတွင် သီးခြား key အဖြစ် register လုပ်ထားခြင်း
    if key == user_id or user_id in auth_list or key in auth_list:
        if user_id in auth_list:
            ...
            # User ID ကို key အဖြစ်ပေးထားသော်လည်း auth_list ထဲမရှိပါက မအတည်ပြုပါ။
            valid = False
...
async def delkey(message):
    ...
    try:
        ...
        await save_auth_list(auth_list)
